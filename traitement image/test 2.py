...
def cercle(image_path):
    ...
    for i in range(len(intersections_merge)):
        ...
        cv2.circle(masque_cercle, (x, y), 10, 255, -1)
        ...
        niveau_gris_intersections.append(moyenne_pixels_dans_cercle)
...
def etat_position(niveau_gris_intersections):
    ...
    for moy, x, y in range (len(niveau_gris_intersections)):
        ordonnée=i//19
        abscisse=i-ordonnée*19
        if 0.0145<niveau_gris_intersections[i]<0.015:
            etat="EM"
        elif 0.017<niveau_gris_intersections[i]:
            etat="B"
        else:
            etat="W"
